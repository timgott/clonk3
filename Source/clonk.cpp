...
char *PrgInfoLine = "RedWolf Design CLONK 3.5 Radikal";
...
char *RankName[11] = { "Clonk", "Ensign", "Lieutenant", "Captain", "Major", "Lieutenant Colonel", "Colonel", "Brigade General", "Major General", "Lieutenant General", "General" };

char *CGModeName[4] = { "Gold mine", "Monsterkill", "Wipf rescue", "Settlement" };

char *CGModeDesc[4] = { "Your objective is to mine all the gold there is. Only the forces of nature stand in your way.",
...
char *EliminationName[3] = { "Kill the Captain", "Eliminate crew", "Capture the flag" };

char *HomeBaseName[2] = { "Is the balloon", "Is the castle " };

char *SModeName[3] = { "Mission", "Cooperative", "Melee" };
char *RuleSetName[3] = { "Easy", "Medium", "Radical" };
BYTE RuleSetCol[3] = { CDGreen, CDBlue, CDRed };

char *SoundStatusName[4] = { "Deactivated", "using CT-VOICE", "using DSP/Timer", "using SDL" };

char *ConTypeName[4] = { "Left  ", "Center", "Right ", "Mouse " };

const int MaxDefQuote = 13;

char *DefQuoteSource[MaxDefQuote] = { "This was cool!",                                  // MaxLen!
...
{
	int rval = 0;
	if (BSA.Plr[0].Col > -1) rval++;
	if (BSA.Plr[1].Col > -1) rval++;
	if (BSA.Plr[2].Col > -1) rval++;
	return rval;
}
...
{
	MANINFO *man = NULL;
	if (!CCrewListPlr) { Message("mnum2man: null ccrewlistplr"); return NULL; }
...
void DefaultPlrRanks(void)
{
	...
	char *defplrrank[DefPlrRankNum] = { "Novice", "Beginner", "Esquire", "Lord", "Baron", "Viscount",
...
void ClearPlrRanks(void)
{
	...
	{
		next = crnk->next; delete crnk;
	}
	FirstPlrRank = NULL;
}

char *PlrRankName(int num)
{
	PLAYERRANK *crnk;
	for (crnk = FirstPlrRank; crnk && (num > 0); crnk = crnk->next, num--);
	if (crnk) return crnk->name;
...
MANINFO *NewDefaultMan2List(MANINFO **crew)
{
	...
	if (!crew) { Message("newman2list: null ptr"); return NULL; }

	if (!(ncap = new MANINFO)) return NULL;

	ncap->name[0] = 0;
	ncap->rank = 0; ncap->exp = 0;
	ncap->rnds = 0; ncap->rean = 0;
	ncap->dead = 0;
...
MANINFO *NewMan2List(MANINFO **crew, char *name, int rank, int exp, int rnds, int rean)
{
	...
	if (!(ncap = NewDefaultMan2List(crew))) return NULL;

	SCopy(name, ncap->name, MaxClonkName);
	ncap->rank = rank; ncap->exp = exp;
	ncap->rnds = rnds; ncap->rean = rean;
	ncap->dead = 0;
...
void DeleteCrewList(MANINFO **crew)
{
	...
	if (!crew) { Message("deletecrewlist: null ptr"); return; }

	for (ccap = *crew; ccap; ccap = ncap)
	{
		ncap = ccap->next; delete ccap;
	}
	*crew = NULL;
}
...
void SortCrewList(MANINFO *crew)
{
	...
	do
	{
		...
		for (cmn = crew; cmn && cmn->next; cmn = cmn->next)
			...
			{
				tmnptr1 = cmn; tmnptr2 = cmn->next;
				tmndat1 = *tmnptr1; tmndat2 = *tmnptr2;

				tmnnxt1 = tmndat1.next; tmnnxt2 = tmndat2.next;
				tmndat1.next = tmnnxt2; tmndat2.next = tmnnxt1;

				*tmnptr1 = tmndat2; *tmnptr2 = tmndat1;
...
void AdjustCrew2Round(int plr)
{
	int cnum, addcl;
	MANINFO *cmn;				// Safety
	if (!BSA.Plr[plr].Info) { Message("adjustcrew2round: null bsa.player["); return; }
...
void PrepareTCrew4Round(int plr)
{
	int cnt;
	MANINFO *cmn;				// Safety
	if (!BSA.Plr[plr].Info) { Message("preptcrew4round: null bsa.player["); return; }
...
void ReanimateMan(int mnum)
{
	...
	if (!man) { Message("reanimateman: null man"); return; }
...
int CrewList(int pnum)
{
	...
	if (!plr) { Message("crewlist: null player"); return 0; } // Safety
	CCrewListPlr = plr;
...
void CrewListCellAction(int num, int aid)
{
	...
	if (centr)
		switch (aid)
		{
		case 0: ManInfoEdit(centr); break;
		case 1: ReanimateMan(num); break;
		}
}
...
int TakeOverImportFace(int dummy)
{
	...
	{
		Message("Error while saving|player portrait file FACES.DAT:|Drive write protected"); return dummy;
	}
	ffwdt = 40 * Min(Config.FaceNum + 1, 8); ffhgt = 50 * ((Config.FaceNum + 1 - 1) / 8 + 1);
...
int ImportFace(int dummy)
{
	...
	if (Config.FaceNum >= 16) { Message("A maximum of 16 player|portraits can be stored."); return dummy; }
...
void DeleteFace(int num)
{
	...
	BYTE saverv;
	int ffwdt, ffhgt;
	if (num < DefFaceNum) { Message("This portrait can|not be deleted."); return; }
	for (cplr = FirstPlayer; cplr; cplr = cplr->next)
		if (cplr->face == num) cplr->face = 0;
	for (cnt = num; cnt < Config.FaceNum - 1; cnt++)
	{
		LPage(FacePage); DBox(40 * (cnt % 8), 50 * (cnt / 8), 40 * (cnt % 8) + 39, 50 * (cnt / 8) + 49, CBlack);
		SVIGCopy(FacePage, 40 * ((cnt + 1) % 8), 50 * ((cnt + 1) / 8), FacePage, 40 * (cnt % 8), 50 * (cnt / 8), 40, 50, 0);
		for (cplr = FirstPlayer; cplr; cplr = cplr->next)
			if (cplr->face == cnt + 1) cplr->face = cnt;
	}
	Config.FaceNum--;
	AdjustLBCellNum(LID_FACE, Config.FaceNum);
	...
	if (PathProtected(RunningPath))
	{
		Message("Error while saving|player portrait file FACES.DAT:|Drive write protected"); return;
	}
...
BYTE FaceListCellAction(int num, int aid)
{
	if (Inside(num, 0, Config.FaceNum - 1))
		switch (aid)
		{
		case 0: return XRVCLOSE;
		case 1: DeleteFace(num); break;
		}
	return 0;
}
...
PLAYERINFO *NewDefaultPlr2List(void)
{
	...
	if (!(nplr = new PLAYERINFO)) return NULL;

	nplr->name[0] = 0; nplr->nick[0] = 0;
	SCopy("I'm a new player.", nplr->quote, MaxQuoteLen);
	nplr->rank = 0;
	for (cnt = 0; cnt < 3; cnt++)
	{
		nplr->rnds[cnt] = 0; nplr->won[cnt] = 0; nplr->score[cnt] = 0;
	}
	nplr->scorepot = 0L;
	nplr->playsec = 0L;
	nplr->face = 0;
	nplr->pfcol = 0; nplr->pfcon = 2;
	nplr->inses = 1;
	nplr->inrnd = 0;
	nplr->pfgpos = 0;
...
{
	if (plr->rnds[smode] == 0) return 0.0;
...
{
	if (PlrTotalRounds(plr) == 0) return 0.0;
...
void DrawPlrListCell(int num, int tx, int ty, BYTE sel)
{
	...
	if (!cplr) return;
	if (cplr->inses) SVIGCopy(MenuPage, 240, 50, MainPage, tx + 1, ty + 1, 40, 50, 0);
...
int SortPlayerList(int dummy) // Makes any stored pnum invalid!
{
	...
	do
	{
		...
		for (cplr = FirstPlayer; cplr && cplr->next; cplr = cplr->next)
		{
			...
			{
			case 0: // Total Score
				if (PlrTotalScore(cplr) < PlrTotalScore(cplr->next)) swtch = 1;
				break;
			case 1: case 2: case 3: // Mission/Cooperate/Melee Score
				if (cplr->score[Config.SortPlrBy - 1] < cplr->next->score[Config.SortPlrBy - 1]) swtch = 1;
				break;
			case 4: // Total Average
				if (PlrTotalAverage(cplr) < PlrTotalAverage(cplr->next)) swtch = 1;
				break;
			case 5: case 6: case 7: // Mission/Cooperate/Melee Average
				if (PlrAverage(cplr, Config.SortPlrBy - 5) < PlrAverage(cplr->next, Config.SortPlrBy - 5)) swtch = 1;
				break;
			}
			if (swtch)
			{
				tplrptr1 = cplr; tplrptr2 = cplr->next;
				tplrdat1 = *tplrptr1; tplrdat2 = *tplrptr2;

				tplrnxt1 = tplrdat1.next; tplrnxt2 = tplrdat2.next;
				tplrdat1.next = tplrnxt2; tplrdat2.next = tplrnxt1;

				*tplrptr1 = tplrdat2; *tplrptr2 = tplrdat1;
...
int PlrEmptyNameCheck(int pnum) // Called on close of PlayerInfEdit
{
	...
	if (cplr->name[0] == 0) SCopy("Player with no name", cplr->name);
	return 0;
}
...
{
	PLAYERINFO *plr = PNum2Plr(pnum);
	if (plr) if (plr->crew) if (!plr->crew->name[0])
...
int NewPlayer2List(int dummy)
{
	...
	{
		Message("Out of memory!|No new players."); return dummy;
	}
...
BYTE ReadFile2NewScriptInfo(FILE *fhnd) //char *fname)
{
	...
	if (!(nscr = new SCRIPTINFO)) return 0;
	...
	if (!ReadFileInfoLine(fhnd, "Head=", OSTR)) fokay = 0;
	if (!SEqual(OSTR, "Clonk3Script")) fokay = 0;
	if (!ReadFileInfoLine(fhnd, "Title=", nscr->title, ScrTitleLen)) fokay = 0;
	if (!ReadFileInfoLine(fhnd, "Desc=", nscr->desc, ScrDescLen)) fokay = 0;
	if (!ReadFileInfoLine(fhnd, "IDNum=", OSTR)) fokay = 0;
	nscr->idnum = strtol(OSTR, NULL, 10);
	if (!Inside(nscr->idnum, 1, 999) || ScriptIDNumExists(nscr->idnum)) fokay = 0;
	if (!ReadFileInfoLine(fhnd, "RuleSet=", OSTR)) fokay = 0;
	nscr->ruleset = strtol(OSTR, NULL, 10);
	if (!Inside(nscr->ruleset, 0, 2)) fokay = 0;

	//fclose(fhnd);
	if (!fokay) { delete nscr; return 0; }

	nscr->next = FirstScript; FirstScript = nscr;
	return 1;
}
...
void SortScriptList(void)
{
	...
	do
	{
		...
		for (scr = FirstScript; scr && scr->next; scr = scr->next)
			...
			{
				scrptr1 = scr; scrptr2 = scr->next;
				scrdat1 = *scrptr1; scrdat2 = *scrptr2;
				tmnnxt1 = scrdat1.next; tmnnxt2 = scrdat2.next;
				scrdat1.next = tmnnxt2; scrdat2.next = tmnnxt1;
				*scrptr1 = scrdat2; *scrptr2 = scrdat1;
				change = 1;
			}
	} while (change);
}
...
int InitScriptList(void)
{
	...
	SortScriptList();
	MissionScriptNum = 0; if (rnum == 0) MissionScriptNum = -1;
	MissionScript.idnum = -1;
	if (BSA.NextMission == 0) if (FirstScript) BSA.NextMission = FirstScript->idnum;
	return rnum;
}
...
void DrawScriptListCell(int num, int tx, int ty, BYTE sel)
{
	SCRIPTINFO *scr = NULL;
	BYTE hide;
	if (num > -1) for (scr = FirstScript; (num > 0) && scr; scr = scr->next, num--);
...
BYTE ScriptListCellAction(int num, int aid)
{
	...
	if (scr)
		switch (aid)
		{
		case 0: return 1; // XRV Start round
		default: Message("undef. aid on scrlistcellact");
		}
	return 0;
}
...
void ClearBSAPlrPtrs(void)
{
	...
	for (cnt = 0; cnt < MaxGamePlr; cnt++)
	{
		BSA.Plr[cnt].Info = NULL; BSA.Plr[cnt].TCrew = NULL; BSA.Plr[cnt].Col = -1;
	}
}
...
int RealismValue(int dummy)
{
	...
	total += 10; rval += rlm->StrcSnow * 10;
	total += 25; rval += rlm->StrcBurn * 25;
	total += 25; rval += rlm->StrcEnrg * 25;
	total += 50; rval += !rlm->EmrPromo * 50;
	total += 25; rval += !rlm->WtrOut * 25;
	total += 10; rval += rlm->RckOut * 10;
	total += 20; rval += !rlm->CstHome * 20;
	dummy++;
...
int DifficultyValue(int dummy)
{
	...
	total += 30; rval += (BSA.RuleSet + 1) * 10;
	total += 10; rval += !BSA.AllowSurrender * 10;

	total += 20; rval += 20 * (6 - BSA.Plr[0].Clonks) / 5; // assumes all plrs equal...
	total += 40; rval += 40 * (200 - BSA.Plr[0].Wealth) / 200;
	total += 15; rval += !BSA.Plr[0].ReadyBase[0] * 15;
	total += 10; rval += !BSA.Plr[0].ReadyBase[1] * 10;
	total += 10; rval += !BSA.Plr[0].ReadyBase[2] * 10;
	total += 5;  rval += !BSA.Plr[0].ReadyBase[3] * 5;
	total += 5;  rval += !BSA.Plr[0].ReadyBase[4] * 5;
	total += 3;  rval += !BSA.Plr[0].ReadyBase[0] * 3;
	total += 5;  rval += !BSA.Plr[0].ReadyBase[1] * 5;
	total += 5;  rval += !BSA.Plr[0].ReadyBase[2] * 5;
	total += 5;  rval += !BSA.Plr[0].ReadyBase[3] * 5;
	total += 15; rval += !BSA.Plr[0].ReadyBase[4] * 15;

	gmfact = 1; if ((BSA.SMode == S_COOPERATE) && (BSA.CoopGMode == C_WIPFRESCUE)) gmfact = 5;
	total += gmfact * 15; rval += gmfact*BSA.Wipfs*1.5;
	total += 30; rval += BSA.Sharks*3.0;
	gmfact = 1; if ((BSA.SMode == S_COOPERATE) && (BSA.CoopGMode == C_MONSTERKILL)) gmfact = 3;
	total += gmfact * 50; rval += gmfact*BSA.Monsters*5.0;

	gmfact = 1; if ((BSA.SMode == S_COOPERATE) && (BSA.CoopGMode == C_GOLDMINE)) gmfact = 3;
	total += gmfact * 19; rval += gmfact*(BSA.BSize - 160) / 10;
	total += 15; rval += 15 * BSA.BCrvAmp / 100;
	total += 15; rval += 15 * BSA.BRndAmp / 100;
	total += 20; rval += 20 * BSA.BWatLvl / 100;
	total += 15; rval += 15 * BSA.BVolcLvl / 100;
	total += 10; rval += 10 * BSA.BQuakeLvl / 100;
	total += 10; rval += 10 * BSA.BCanyon;

	total += 15; rval += 15 * Abs(BSA.WClim - 500) / 500;
	total += 15; rval += 15 * BSA.WYSpd / 100;
	total += 20; rval += 20 * (BSA.WRFMod + 50) / 100;
	total += 20; rval += 20 * (BSA.WLPMod + 50) / 100;
	total += 15; rval += 15 * BSA.WCmtLvl / 100;
	total += 15; rval += 15 * (BSA.WEnvr == 1);

	total += 200; rval += 200 * (20 - BSA.FlintRadius) / 10;
	total += 200; rval += 200 * (5 - BSA.RckProdSpeed) / 4;
...
void RndInitBackground(void)
{
	...
	BSA.BCrvAmp = 0; BSA.BRndAmp = rsper; BSA.BCanyon = 0;

	if (!random(2))
	{
		BSA.BCrvAmp = random(rsper); BSA.BRndAmp = 20 + random(80);
		BSA.BPhase = random(100); BSA.BPLen = random(100);
		if (BSA.RuleSet > R_EASY)
			if (!random(10)) BSA.BCanyon = 1;
	}

	BSA.BWatLvl = 0; BSA.BVolcLvl = 0; BSA.BQuakeLvl = 0;
	if (BSA.RuleSet > R_EASY)
	{
		if (!random(6)) BSA.BVolcLvl = random(101);
		if (!random(12)) BSA.BQuakeLvl = random(101);
	}
	if (!random(3) || BSA.Sharks) BSA.BWatLvl = random(Max(BSA.BCrvAmp, BSA.BRndAmp));
	...
	if (BSA.SMode == S_MELEE)
	{
		if (!random(3))
		{
			BSA.BRndAmp = 20; BSA.BCrvAmp = 40 + random(BSA.RuleSet * 30);
			if (BSAPlrNum() == 2) // Hill/Valley
			{
				BSA.BPhase = 70; BSA.BPLen = 53;
			}
			if (BSAPlrNum() == 3) // Triple Hill/Valley
			{
				BSA.BPhase = 75; BSA.BPLen = 100;
			}
			BSA.BWatLvl = random(100); if (!random(2)) BSA.BWatLvl = 70 + random(30);
		}
	}
}
...
{
	if (BSA.RuleSet == R_EASY) BSA.Sharks = 0;
	else BSA.Sharks = random(4 + 3 * BSA.RuleSet);
...
void RndInitWeather(void)
{
	if (BSA.RuleSet == R_EASY)
	{
		BSA.WClim = 300 + 600 * random(2); BSA.WSeas = 750 + 1000 * random(2); BSA.WYSpd = 0;
	}
	else
	{
		BSA.WClim = random(1000); BSA.WSeas = random(2000); BSA.WYSpd = random(101);
	}
	BSA.WRFMod = random(100) - 50; BSA.WLPMod = random(2)*(random(60) - 50);
	if ((BSA.RuleSet == R_EASY) || !random(3))
	{
		if (!random(2)) BSA.WRFMod = -50; if (!random(2)) BSA.WLPMod = -50;
	}
	BSA.WCmtLvl = random(2)*random(10 + 20 * BSA.RuleSet);
	BSA.WEnvr = 0; if (BSA.RuleSet > R_EASY) if (!random(10)) BSA.WEnvr = 1 + random(2);
}
...
void DefaultBSA(BYTE newinit)
{
	...
	BSA.GPlrElm = 0;
	// Players
	for (cnt = 0; cnt < MaxGamePlr; cnt++) DefBSAPlayer(cnt, newinit);
	// Animals
	BSA.AnmRandom = 0;
	BSA.Wipfs = 0;
	BSA.Sharks = 0;
	...
	BSA.BCanyon = 0;
	for (cnt = 0; cnt < 20; cnt++) BSA.BRockType[cnt] = 0; // ROCK
	BSA.BRockMode = 0;
	// Weather
	BSA.WeaRandom = 0;
	...
	BSA.FlintRadius = 12;
	// Realism
	BSA.Realism.StrcSnow = 0; BSA.Realism.StrcBurn = 1;
...
int AdjustBSA2SMode(int dummy)
{
	...
	case S_COOPERATE:
		...
		if (BSA.RckProdSpeed == 0) BSA.RckProdSpeed = 1;
		BSA.GPlrElm = 1;
		BSA.AllowSurrender = 1;
		break;
	case S_MELEE:
...
{			          // if switching rule sets
	...
	case R_EASY:
		...
		BSA.Plr[0].Wealth = 0;
		for (cnt = 1; cnt < 5; cnt++) BSA.Plr[0].ReadyBase[cnt] = 0;
		for (cnt = 2; cnt < 5; cnt++) BSA.Plr[0].ReadyVhc[cnt] = 0;
	...
	case R_ADVANCED:
		// Unchooseable (required)
		BSA.Plr[0].ReadyBase[2] = 0; BSA.Plr[0].ReadyBase[3] = 0;
		for (cnt = 3; cnt < 5; cnt++) BSA.Plr[0].ReadyVhc[cnt] = 0;
		if (setdef)
		{
			// Chooseable (default only)
			BSA.Plr[0].Wealth = 20;
...
void AdjustBSA2CoopGMode(void)
{
	...
	case C_MONSTERKILL: // Adjust animal presence
		if (BSA.Monsters == 0) BSA.Monsters = 1 + random(2 + 2 * BSA.RuleSet);
		break;
	case C_WIPFRESCUE:
		if (BSA.Wipfs == 0) BSA.Wipfs = 3 + random(5 + 5 * BSA.RuleSet);
		break;
	}
}
...
{                                     // Hardcoded stuff
	int jumpto = -1;
	Capitalize(OSTR);
	if (SEqualL(OSTR, "WIPFEMONSTERSCHNEE")) jumpto = 13;
	if (SEqualL(OSTR, "VULKANBEBENAUSBRUCH")) jumpto = 23;
	if (SEqualL(OSTR, "DASWARHARTEARBEIT")) jumpto = 30;
	if (jumpto > -1)
...
int OpenUserPrefWindow(int ingame)
{
	...
	if (ingame) NewButton("Close", 0, 220, 125, 40, 9, XRVCLOSE, 0);
	if (!ingame)
		NewULink(EXCREDR, NULL,
...
{
	if ((BSA.Plr[cplr].Col == -1) || (!BSA.Plr[cplr].Info)) return;
...
BYTE InitCSBMenuPages(void)
{
	...
	// Realism Page ------------------------------------------------------------
	if (BSA.SMode > S_MISSION) if (BSA.RuleSet > R_EASY)
	{
		if (BSA.RuleSet < R_RADICAL) hd = 1; else hd = 0;
	...
	if (BSA.SMode > S_MISSION)
	{
		...
		for (cnt = 4; cnt >= 0; cnt--)
		{
			hd = 0; if (((BSA.RuleSet < R_ADVANCED) && (cnt > 1)) || ((BSA.RuleSet < R_RADICAL) && (cnt > 2))) hd = 1;
		...
		{
			hd = 0;
			if ((BSA.RuleSet == R_EASY) && (cnt > 0)) hd = 1;
			if ((BSA.RuleSet == R_ADVANCED) && Inside(cnt, 2, 3)) hd = 1;
	...
	if (InitErrorCheck()) { ClearSVI(); return 0; }
...
BYTE InitEndPrPage(DWORD modify)
{
	...
	for (cnt = 0; cnt < MaxGamePlr; cnt++)
	{
		if (BSA.Plr[cnt].Col > -1) if (BSA.Plr[cnt].Info) // latter one safety
		{
...
void DrawListBoxCell(int lid, int num, int tx, int ty, BYTE sel)
{
	switch (lid)
	{
	case LID_CAPS: DrawCrewListCell(num, tx, ty, sel); break;
	case LID_PLRS: DrawPlrListCell(num, tx, ty, sel);  break;
	case LID_FACE: DrawFaceListCell(num, tx, ty, sel); break;
	case LID_SCRS: DrawScriptListCell(num, tx, ty, sel); break;
	default:
...
BYTE ListBoxCellAction(int lid, int num, int aid)
{
	switch (lid)
	{
	case LID_CAPS: CrewListCellAction(num, aid); break;
	case LID_PLRS: PlrListCellAction(num, aid); break;
	case LID_FACE: return FaceListCellAction(num, aid);
	case LID_SCRS: return ScriptListCellAction(num, aid);
	default:
...
BYTE RoundMenuAbort(void)
{
	...
	if (!InitErrorCheck()) if (RunSVI() == 1) rval = 1;
	ClearSVI();
...
void EnterNewPlrRank(void)
{
	...
	if (!nrank[0]) sprintf(nrank, "Clonk Number %d", PlrRankCount() + 1);
	NewPlrRank(nrank);
}
...
void PlayerPromotionCheck(void)
{
	...
	for (plr = 0; plr < MaxGamePlr; plr++)
		if (BSA.Plr[plr].Info)
		{
			...
			if (cplr->rank + 1 >= PlrRankCount()) EnterNewPlrRank();
			...
			if (cplr->rank < PlrRankCount())
				if (PlrTotalScore(cplr) >= PlayerRankScore(cplr->rank + 1))
				{
					...
					CTextObject(PlrRankName(cplr->rank), 60, 101, CDBlue, -1, 1);

					NewObject("Congrats!", BUTTON, 1, 0, 25, 127, 70, 9, XRVCLOSE, 0, 0, 0, 0, 0, 0, 0, NULL, NULL);

					if (!InitErrorCheck()) RunSVI();
					ClearSVI();
...
void RoundEndProcess(PLAYERINFO **winnerp)
{
	...
	case S_COOPERATE:
		...
		if (BSA.CoopGMode == C_WIPFRESCUE)
			if (BSA.Wipfs == 0) Message("Error: Wipf-Rescue mode should|have had wipfs!");
			else modify = modify*WipfResc / BSA.Wipfs;
			break;
	case S_MELEE:
		modify = 100L; // 100%
		break;
	...
	for (plr = 0; plr < MaxGamePlr; plr++)
		if (BSA.Plr[plr].Col > -1) if (BSA.Plr[plr].Info)
		{
			if (BSA.PCrew)
				for (cmn = BSA.Plr[plr].Info->crew; cmn; cmn = cmn->next)
				{
					if (!cmn->dead) cmn->rnds++; // Only survived rounds count
					if (BSA.Plr[plr].Info->autorean) if (cmn->dead)
	...
	if (InitEndPrPage(modify)) { RunSVI(); ClearSVI(); }
...
BYTE CustomSetBSA(void)
{
	...
	if (!LoadMenuPages()) return 0;

	DrawTitleBack(MainPage, 0);
	do
	{
		retryset = 0;
		if (!InitCSBMenuPages()) return 0;
		svir = RunSVI();
		ClearSVI();
		if (svir != 1) return 0; // Session aborted
	...
	// Randomize Randoms
	if (BSA.BckRandom) RndInitBackground();
	if (BSA.WeaRandom) RndInitWeather();
	if (BSA.AnmRandom) RndInitAnimals();

	// Adjust 2 CoopGMode
	if (BSA.SMode == S_COOPERATE) AdjustBSA2CoopGMode();
...
BYTE MainMenus(void) // Returns  0 Exit  1 GmeSession
{
	...
	if (FirstTimeRun) { OpenPowerHelp("welcome"); RunSVI(); }

	if (PlrListCount() < 1) { NewPlayer2List(0); RunSVI(); }

	if (FirstTimeRun)
		if (SBDetected) { SoundSetup(0); RunSVI(); }
		else ConfirmedCall("Is a sound blaster compatible|sound card installed on this system?", 2, &SoundSetup, 0);

		if (!InitRosterMenuPages()) return 0;
...
BYTE ExecRoundOkay(void)
{
	...
	case S_MISSION:
		...
		{
			Message("No mission has been selected!", NULL); return 0;
		}
		break;
	case S_MELEE:
		if (BSA.RuleSet == R_EASY)
			if (BSA.GPlrElm == 2)  if (!BSA.Plr[0].ReadyBase[0])
			{
				Message("In the EASY rule set, capture the flag|can not be played without|a castle home base.", "round starting conditions"); return 0;
			}
		break;
	case S_COOPERATE:
		if (BSA.RuleSet == R_EASY)
			switch (BSA.CoopGMode)
			{
			case C_GOLDMINE: if (!BSA.Plr[0].ReadyBase[0])
			{
				Message("In the EASY rule set, gold mine|can not be played without|a castle home base.", "round starting conditions"); return 0;
			}
							 break;
			case C_WIPFRESCUE: if (!BSA.Plr[0].ReadyBase[0] || !BSA.Plr[0].ReadyVhc[0])
			{
				Message("Int the EASY rule set, you need|a castle home base and a lorry|if you want to rescue wipfs.", "round starting conditions"); return 0;
			}
							   break;
			}
		break;
...
{
	if (PlrsInSession(0) < 1) { Message("Please mark all players who want|to take part in the new session.", NULL); return 0; }
	switch (BSA.SMode)
	{
	case S_MISSION:
		if (PlrsInSession(0) > 1) { Message("The tutorial rounds can only be played|by one player at a time.", NULL); return 0; }
		break;
	case S_COOPERATE:
		if (!Inside(PlrsInSession(0), 1, 3)) { Message("In cooperative mode, a maximum of|three players per round can participate.", NULL); return 0; }
		break;
	case S_MELEE:
		if (PlrsInSession(0) < 2) { Message("A single player may not|play in melee mode.", NULL); return 0; }
		break;
	}
	return 1;
}
	...
	for (cplr = FirstPlayer, pin = 0; cplr && (pin < MaxGamePlr); cplr = cplr->next)
		if (cplr->inrnd)
		{
			...
			for (cnt = 0, pused = 0; cnt < MaxGamePlr; cnt++)
				if (BSA.Plr[cnt].Col > -1) if (BSA.Plr[cnt].Con == pcon) pused = 1;
			if (pused) pcon = 0;
			do
			{
				for (cnt = 0, pused = 0; cnt < MaxGamePlr; cnt++)
			...
			{
				ppos = pcon; if (ppos > 2) ppos = 2;
			}                // gpos placement
			else // By random                                   // priority
				ppos = random(MaxGamePlr);
			while (BSA.Plr[ppos].Col > -1) ++ppos %= MaxGamePlr;
			...
			{
				for (cnt = 0, pused = 0; cnt < MaxGamePlr; cnt++)
					if (BSA.Plr[cnt].Col == plrc) pused = 1;
				if (pused) ++plrc %= 8;
			} while (pused);
...
{                 // Will try to get up to 3 players in session for inrnd
	...
	*oldestp = NULL;
	for (cplr = FirstPlayer, cnt = 3; cplr; cplr = cplr->next)
	{
		...
		if (cplr->inses && (cnt > 0))
		{
			cplr->inrnd = 1; cnt--; if (!(*oldestp)) *oldestp = cplr;
		}
	}
}
...
void NewPlayersInRnd(PLAYERINFO **oldestp, PLAYERINFO *winnerp)
{
	...
	case 0: // Rotation: oldest out, new in
		cplr = (*oldestp)->next;
		(*oldestp)->inrnd = 0; *oldestp = NULL;
		for (exok = 0; !exok; cplr = cplr->next)
		{
			if (!cplr) cplr = FirstPlayer;
			if (!(*oldestp)) if (cplr->inses && cplr->inrnd) *oldestp = cplr;
			if (cplr->inses && !cplr->inrnd) { cplr->inrnd = 1; exok = 1; }
		}
		break;
	case 1: // Challenge: winner stays, others out
		...
		{
			if (!cplr) cplr = FirstPlayer;
			if (cplr->inses && !cplr->inrnd) { *oldestp = cplr; exok = 1; }
		}
		for (cplr = FirstPlayer; cplr; cplr = cplr->next)
			if (cplr != winnerp) cplr->inrnd = 0;
		for (cplr = *oldestp, cnt = 2; cnt > 0; cplr = cplr->next)
		{
			if (!cplr) cplr = FirstPlayer;
			if (cplr->inses && !cplr->inrnd) { cplr->inrnd = 1; cnt--; }
		}
		break;
	}
}
...
void GameSession(void)
{
	...
	while (CustomSetBSA())
		if (ExecRoundOkay())
		{
			if (!oldestp) Message("System Error: oldestp should|never be null!");
			...
			MenuPagesLoaded = 0; LPage(MainPage); PPage(MainPage);
			InitSVIMouse();

			if (aborted == 2) Message("Insufficient memory|can't start the round with these settings", "insufficient memory for round");
			RoundErrorMessage();

			if (!aborted) { RoundEndProcess(&winnerp); Config.RoundEndCount++; }

			CheckCrewLists();

			if (Config.SaveDatAlways) SaveConfigSystem();

			// If MELEE tournament, get new players
			if (PlrsInSession(0) > 3) if (!aborted)
	...
	if (BSA.SMode == S_MISSION) DefaultBSA(0); // Reset from extremes
...
void MainPrg(void)
{
	...
	if (FIntegCheck()) InitMsg("Ok"); else pfail = 1;
...
int main(int argc, char *argv[])
{
	...
	{
		InitMsg(PrgInfoLine, CRed); InitMsg("");
