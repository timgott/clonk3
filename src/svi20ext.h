...
extern void InitMsg(const char *msg, BYTE col=CIMsg);
extern void InitMsgOpen(const char *msg, BYTE col=CIMsg);

extern void SVISetExternFunctions(void(*dlbc)(int,int,int,int,BYTE),BYTE(*lbca)(int,int,int));

extern OBJECT *NewObject(const char *title, BYTE type, BYTE acc, BYTE hid, int x, int y, int wdt, int hgt, BYTE bd1, BYTE bd2, BYTE bd3, int id1, int id2, int id3, int id4, int id5, char *bptr, int *dptr);
extern WINDOW *NewWindow(const char *title, BYTE type, BYTE prior, int x, int y, int wdt, int hgt, int num, int outof, BYTE escxrv, char *hindex);
extern ULINK  *NewULink(BYTE type, WINDOW *fwin, OBJECT *fobj, OBJECT *tobj, int (*exfunc)(int), int expar);

extern OBJECT *TextObject(const char *txt, int tx, int ty, int fcol, int bcol=-1, BYTE form=0);
extern OBJECT *CTextObject(char *txt, int tx, int ty, int fcol, int bcol=-1, BYTE form=0);
extern OBJECT *ITextObject(const char **txt, int *index, int tx, int ty, int fcol, int bcol=-1, BYTE form=0);
extern OBJECT *NewButton(const char *title, BYTE hide, int tx, int ty, int wdt, int hgt, BYTE xrval, BYTE noulr);
extern OBJECT *NewFlagBox(const char *title, int tx, int ty, int *valptr, BYTE hide=0);
...
extern void Message(const char *txt, char *hindex=NULL);
extern void ConfirmedCall(const char *txt, int ctype, int (*exfunc)(int), int expar, char *hindex=NULL);
extern void LineInput(const char *msg, char *txt, int maxlen, int (*exfunc)(int)=NULL, int expar=0, char *hindex=NULL);
...
extern void InitProcessB(const char *msg, BYTE page=SVIPage);
